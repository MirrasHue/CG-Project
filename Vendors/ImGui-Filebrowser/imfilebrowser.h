...
namespace ImGui
{
    ...
    public:
        ...
        // returns true when there is a selected filename and the "Open" button was clicked
...
inline void ImGui::FileBrowser::Display()
{
    ...
    {
        if(!BeginPopup(openLabel_.c_str(), ImGuiWindowFlags_AlwaysAutoResize))
        {
            return;
        }
    }
    ...
    if(newDirNameBuf_)
    {
        ...
        if(BeginPopup(openNewDirLabel_.c_str()))
        {
            ...
            if(Button("Open") && (*newDirNameBuf_)[0] != '\0')
            {
    ...
    if(!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
    {
        if(Button(" Open ") && !selectedFilenames_.empty())
        {
            ok_ = true;
            CloseCurrentPopup();
        }
